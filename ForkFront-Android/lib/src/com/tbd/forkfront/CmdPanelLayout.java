	...
	private void loadPanels(SharedPreferences prefs)
	{
		...
		if(panelsChanged(mPanelCmds, panelCmds))
		{
			...
			for(int i = panelCmds.size() - 1; i >= 0; i--)
			{
				...
				{
					android.util.Log.e("ForkFront", "Failed to create command panel " + panel.idx, t);
					((ViewGroup)panel.portScrollView.getParent()).removeView(panel.portScrollView);
					if(panel.portScrollView != panel.landScrollView)
						((ViewGroup)panel.landScrollView.getParent()).removeView(panel.landScrollView);
					panelCmds.remove(i);
					continue;
				}
				if(mPortraitMode)
